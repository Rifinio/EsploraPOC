...
class MockBlockchainStore: BlockchainStoreProtocol {
    ...
    func loadWallets(completion: @escaping ([Wallet]) -> Void) {
        let wallets = MockBlockchainStore.addresses.map { return Wallet(address: $0)}
        completion(wallets)
    }
    
    func loadWallet(address: String, completion: @escaping (Wallet?) -> Void) {
        completion(Wallet(address: "fake-address"))
    }
}
